...
{
    if(!is_valid(i, j) || a[i][j] == '#' || vis[i][j]) return false;
    if(i == n - 1 && j == m - 1) return true;

    vis[i][j] = 1;

    if(exit(i, j + 1)) return true;
    if(exit(i + 1, j)) return true;
    if(exit(i - 1, j)) return true;
    if(exit(i, j - 1)) return true;
...
void Solve()
{
    ...
    rep(i, 0, n) rep(j, 0, m)
        if(a[i][j] == 'B')
            rep(k, 0, 4)
                if(is_valid(i + dx4[k], j + dy4[k]))
                {
                    if(a[i + dx4[k]][j + dy4[k]] != 'G' && a[i + dx4[k]][j + dy4[k]] != 'B') a[i + dx4[k]][j + dy4[k]] = '#';
                    else if(a[i + dx4[k]][j + dy4[k]] == 'G') { cout << "No" << endl; return; }
                }
    ...
    rep(i, 0, n) rep(j, 0, m)
        if(a[i][j] == 'G')
        {
            ...
            if(!exit(i, j)) { cout << "No" << endl; return; }
        }

...
clock_t start_time;
string get_run_time()
{ return '\n' + to_string((clock() - (double)start_time) / CLOCKS_PER_SEC) + " sec\n"; }
...
const int N = (int)3e5 + 9, M = (int)2e6 + 9;

/**----------------->>  Quality Over Quantity  <<----------------**/

int next_greater[N], next_smaller[N], prev_greater[N], prev_smaller[N], Head[N], Next[M], To[M], ne, H[N], n, dis[N];
stack <int> st;
...
void Solve()
{
	...
	for(int i = n; i; --i) // next greater
    {
        ...
        if(st.empty()) next_greater[i] = -1;
        else next_greater[i] = st.top();
        st.push(i);
    }
    ...
    for(int i = 1; i <= n; ++i) // previous greater
    {
        ...
        if(st.empty()) prev_greater[i] = -1;
        else prev_greater[i] = st.top();
        st.push(i);
    }
    /**
    write(next_greater + 1, next_greater + n + 1);
    write(next_smaller + 1, next_smaller + n + 1);
    write(prev_greater + 1, prev_greater + n + 1);
    write(prev_smaller + 1, prev_smaller + n + 1);
    4
    1 4 5 2
    */
    for(int i = 1; i <= n; ++i)
    {
        if(next_greater[i] != -1) addEdge(i, next_greater[i]);
        if(next_smaller[i] != -1) addEdge(i, next_smaller[i]);
        if(prev_greater[i] != -1) addEdge(prev_greater[i], i);
        if(prev_smaller[i] != -1) addEdge(prev_smaller[i], i);

...
const int    MxsZ = (int)1e5 + 100, mod = 1073741824,         oo = 0x3f3f3f3f;

/**----------------------->>  Template  <<------------------------*/

int n, arr[MxsZ], dol, enr;
...
int main()
{
    ...
    cin >> n; read(arr, 1, n + 1);
    rep(i, 0, n)
    {
        if(arr[i] - arr[i + 1] + enr < 0) dol += abs(arr[i] - arr[i + 1] + enr), enr = 0;
        else enr += arr[i] - arr[i + 1];
    }
    RT(dol);
}

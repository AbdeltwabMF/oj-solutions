...
const int    MxsZ = (int)1e7    ,   mod = 1073741824,         oo = 0x3f3f3f3f;

/**----------------------->>  Template  <<------------------------*/

int n, m, a, cnt[MxsZ + 1], pre[MxsZ + 1], l, r;

void countPrimes(int n)
{
    rep(i, 0, n) { cin >> a; cnt[a]++; }

    vector <bool> visited (MxsZ + 1);
    for(int i = 2; i <= MxsZ; i++)    if(!visited[i])
    {
        for(int j = i; j <= MxsZ; j += i) { if(cnt[j]) pre[i] += cnt[j]; if(j > i) visited[j] = true; }
    }
    rep(i, 2, MxsZ + 1) { pre[i] += pre[i - 1]; }
}
...
int main()
{
    ...
    while(m--)
    {
         cin >> l >> r; if(l > MxsZ) l = MxsZ + 1;
         cout << pre[min(r, MxsZ)] - pre[l - 1] << endl;
    }
}

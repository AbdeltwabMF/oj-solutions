...
void Solve()
{
    ...
    pair <int, int> vertices[n];
    for(int i = 1; i <= n; ++i) vertices[i - 1] = {degree[i], i};

    sort(vertices, vertices + n, greater <pair <int, int> > ());
    vector < pair <int, int> > st;

    for(int i = 0; i < n; ++i)
        for(int j = Head[vertices[i].second]; j; j = Next[j])
            if(uf.union_set(vertices[i].second, To[j]))
                st.emplace_back(vertices[i].second, To[j]);

...
int n, ans, m, any_zero;
string s;
bool any_even;

void solve()
{
    cin >> s; m = sz(s); any_even = any_zero = ans = 0;

    rep(i, 0, m)
    {
        if((s[i] - '0') % 2 == 0 && s[i] != '0') any_even = true;
        else if(s[i] == '0') any_zero++;
        ans += (s[i] - '0');
    }
    if(!any_even && any_zero > 1) any_even = 1;
    cout << ((any_zero && any_even && !(ans % 3)) ? "red" : "cyan") << endl;
}

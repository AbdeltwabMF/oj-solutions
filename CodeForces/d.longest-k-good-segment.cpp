...
int main()
{
    ...
    cin >> n >> k; rep(i, 0, n) cin >> arr[i];

    while(r < n)
    {
       if(sz(st) <= k)
       {
           fr[arr[r]]++; st.insert(arr[r++]);
       }
       else
       {
           if(r-l-1 > get <2> (ans)) get <0> (ans) = l+1, get <1> (ans) = r-1, get <2> (ans) = r-l;

           if(fr[arr[l]] > 1) fr[arr[l++]]--;
           else st.erase(arr[l]), fr[arr[l++]]--;
       }
       if(sz(st) <= k && (r-l > get <2> (ans)))
            get <0> (ans) = l+1, get <1> (ans) = r, get <2> (ans) = r-l;
    }

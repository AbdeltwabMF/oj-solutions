...
{
    int l = _begin - _begin, r = _end - _begin;
...
void Solve()
{
    ...
    for(int i = 0; i < sz(s); ++i)
        if(valid[i] > longest_rbs) longest_rbs = valid[i];

    if(longest_rbs)
    cnt = count_if(valid, valid + sz(s), [&](int val){ return val == longest_rbs; });

    cout << longest_rbs << " " << cnt << endl;
}

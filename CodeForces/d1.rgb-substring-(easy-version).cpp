...
bool com(ll a, ll b) { return (a&1) >  (b&1); };
...
const int         MxsZ       = (int)2e5+5, inf = (int)9e9;

/**----------------------->>  Template  <<------------------------*/

int q, n, k, mn, indx, cnt;
    ...
    while(q--)
    {
        ...
        rep(i, 0, 3)
        {
            ...
            rep(j, 0, n-k+1)
            {
                ...
                rep(c, 0, k)
                {
                    if(store[indx%3] != str[c+j]) cnt++; indx++;
                }
                mn = min(mn, cnt);
            }
        }

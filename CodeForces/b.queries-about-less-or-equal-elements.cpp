...
int main()
{
    ...
    cin >> n >> m; vector <int> a(n), b(m), c(m);
    rep(i, 0, n) cin >> a[i]; rep(i, 0, m) { cin >> b[i]; c[i] = b[i]; } sort(all(a)); sort(all(c));

    while(l2 < m)
    {
        if(a[l1] <= c[l2] && l1 < n) l1++; else ans[c[l2++]] = l1;
    }
    rep(i, 0, m) cout << ans[b[i]] << " ";
}

...
const int         MxsZ       = (int)1e5+6;

/**----------------------->>  Template  <<------------------------*/

int n, d, l, r;
ll sum, res;
pair <int, int> arr[MxsZ];

int main()
{
    DO_NOT_GIVE_UP

    cin >> n >> d; rep(i, 0, n) cin >> arr[i].F >> arr[i].S; sort(arr, arr+n); sum = res = arr[0].S, r++;

    while(r < n)
    {
        if(arr[r].F - arr[l].F < d) sum += arr[r++].S;
        else sum -= arr[l++].S; res = max(res, sum);
    }
    cout << res << endl;
}

...
void Solve()
{
    ...
    for(int i = 0; i < n; ++i)
        if(a[i] > mnA && b[i] > mnB)
            if(a[i] - mnA < b[i] - mnB)
                b[i] -= a[i] - mnA, ans += a[i] - mnA, a[i] = mnA;
            else
                a[i] -= b[i] - mnB, ans += b[i] - mnB, b[i] = mnB;

    cout << ans + accumulate(a, a + n, 0ll) + accumulate(b, b + n, 0ll) - n * (mnA + mnB) << endl;
}

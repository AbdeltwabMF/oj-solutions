...
void Solve()
{
    ...
    read(X, X + n); shandomRuffleSort(X, X + n);
    for(int i = 0; i < n; cin >> Y, ++i);
    memset(seg, 0x00, (n + 2) << 3);

    for(int i = 0; i < n; ++i)
    {
        int lo = i - (lower_bound(X, X + n, X[i] - k) - X) + 1;
        seg[i + 1][0] = max(lo, seg[i][0]);
    }

    for(int i = n - 1; i >= 0; --i)
    {
        int up = (upper_bound(X, X + n, X[i] + k) - X) - i;
        seg[i][1] = max(up, seg[i + 1][1]);
    }

    for(int i = 0; i <= n; ++i) ans = max(ans, seg[i][0] + seg[i][1]);
    cout << ans << endl;
}

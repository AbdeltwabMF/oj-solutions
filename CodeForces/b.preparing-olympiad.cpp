...
int n, l, r, x, arr[18], cnt = 0, v;

void solve(int ind, int curSum, int mn, int mx, int num)
{
    if(ind >= n) {  if(num >= 2 && l <= curSum && curSum <= r && mx - mn >= x) cnt++; return; }

    solve(ind + 1, curSum + arr[ind], min(mn, arr[ind]), max(mx, arr[ind]), num + 1);
    solve(ind + 1, curSum, mn, mx, num);
}
...
int main()
{
    ...
    cin >> n >> l >> r >> x; read(arr, 0, n); //sort(arr, arr+n);
    solve(0, 0, oo, 0, 0);
    cout << cnt << endl;
}

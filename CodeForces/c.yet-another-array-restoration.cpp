...
void Solve()
{
    ...
    vector <int> soFar, factors;
    int bla = y - x;
    factors.push_back(1);
    for(int i = 1; i < 100; ++i)
    {
        if(bla % i == 0)factors.push_back(i);
    }
    ...
    for(int i = 0; i < sz(factors); ++i)
    {
        if(bla / factors[i] <= n)
        {
            ...
            if(sz(soFar) < n)
            {
                for(int j = x - factors[i]; sz(soFar) < n && j > 0; j -= factors[i])
                    soFar.push_back(j);
            }
            if(sz(soFar) < n)
            {
                for(int j = y + factors[i]; sz(soFar) < n; j += factors[i])
                    soFar.push_back(j);
            }

...
int main()
{
    ...
    rep(i, 1, n+1) cin >> arr[i];     sort(arr+1, arr+n+1);

    while(q--)
    {
        cin >> l >> r; par[r+1]--, par[l]++;
    }

    rep(i, 1, n+1) par[i] += par[i-1]; sort(par+1, par+n+1);

    rep(i, 1, n+1) sum += (ll)par[i] * arr[i];
